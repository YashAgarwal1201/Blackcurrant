...
import { OverlayPanel } from "primereact/overlaypanel"; // [NEW]
import { ScrollTop } from "primereact/scrolltop"; // [NEW]
...
import "./WebAPI.scss";
import { Download, EllipsisVertical, Search, X } from "lucide-react"; // [NEW] added Download

// ─── Report generator (Option A — DOM scrape) ──────────────────────── [NEW] ──
/**
 * Walks all rendered [data-card-key] elements in the page container and
 * extracts their visible text into a structured Markdown report.
 *
 * Relies on the DOM shape of ApiCard:
 *   - h3.font-heading           → card title
 *   - .data-row-label           → DataRow label   (we add this class below)
 *   - .data-row-value           → DataRow value   (we add this class below)
 *   - p (purpose line)          → last <p> with border-t class
 *
 * NOTE: We use textContent so live values (e.g. battery %, clock) are
 * captured at the exact moment the user clicks Download.
 */
function generateMarkdownReport(containerEl: HTMLElement): string {
  const now = new Date();
  ...
  const lines: string[] = [
    "# Browser Vitals Report",
    `> Generated: ${timestamp}`,
    `> Source: Blackcurrant — Browser Vitals`,
    "",
    "---",
    "",
  ];

  // Group cards by category heading (use the rendered h2 section headers)
  const sections = containerEl.querySelectorAll<HTMLElement>(
    "section[aria-labelledby]",
  );

  if (sections.length === 0) {
    // Flat fallback — no grouping visible (single category selected)
    const cards = containerEl.querySelectorAll<HTMLElement>("[data-card-key]");
    cards.forEach((card) => appendCardLines(card, lines));
  } else {
    sections.forEach((section) => {
      const headingEl = section.querySelector("h2");
      const iconEl = section.querySelector("span[aria-hidden='true']");
      if (headingEl) {
        const icon = iconEl?.textContent?.trim() ?? "";
        lines.push(`## ${icon} ${headingEl.textContent?.trim() ?? ""}`);
        lines.push("");
      }
      const cards = section.querySelectorAll<HTMLElement>("[data-card-key]");
      cards.forEach((card) => appendCardLines(card, lines));
    });
  }
  ...
  lines.push("---");
  lines.push(
    "_Report generated by Blackcurrant — https://blackcurrant.vercel.app_",
  );

  return lines.join("\n");
}

function appendCardLines(card: HTMLElement, lines: string[]): void {
  ...
  // DataRow pairs — label + value divs with our marker classes
  const rows = card.querySelectorAll<HTMLElement>(".data-row");
  rows.forEach((row) => {
    const label = row.querySelector(".data-row-label")?.textContent?.trim();
    const value = row.querySelector(".data-row-value")?.textContent?.trim();
    if (label && value) {
      lines.push(`- **${label}:** ${value}`);
    }
  });

  // Purpose line (the small <p> at the bottom of the card)
  const purposeEl = card.querySelector<HTMLElement>("p.data-row-purpose");
  if (purposeEl) {
    lines.push("");
    lines.push(`> ${purposeEl.textContent?.trim()}`);
  }

  lines.push("");
}

function triggerMarkdownDownload(containerEl: HTMLElement): void {
  const md = generateMarkdownReport(containerEl);
  const blob = new Blob([md], { type: "text/markdown;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  const ts = new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19);
  a.href = url;
  a.download = `browser-vitals-${ts}.md`;
  a.click();
...
const WebAPI = () => {
  ...
  const handleDownload = () => {
    if (pageRef.current) {
      triggerMarkdownDownload(pageRef.current);
    }
  };

  // [NEW] Shared Download button element (reused in both desktop bar and mobile popover)
  const DownloadButton = (
    <Button
      onClick={handleDownload}
      aria-label="Download browser vitals report"
      title="Download as Markdown"
      className="shrink-0 flex items-center gap-1.5 h-full bg-base-200! border! border-neutral! rounded-lg! text-base-content! font-content transition-colors hover:border-primary! hover:text-primary! active:opacity-70"
    >
  ...
  // [NEW] Shared Dropdown element (reused in both desktop bar and mobile popover)
  const FilterDropdown = (
    <Dropdown
      ...
      valueTemplate={(value: WebApiCategoryMeta) => (
        <span className="text-base-content font-content">{value.label}</span>
      )}
    />
  );
  ...
  return (
    <Layout>
      <div
        ref={pageRef} // [NEW]
        className="web-apis-page custom-scrollbar w-full h-full py-2 md:py-3 lg:py-4 pl-2 md:pl-3 lg:pl-4 flex flex-col gap-y-4 md:gap-y-6 overflow-y-auto"
      >
        {/* Page header */}
        ...
        <div className="web-apis-controls flex flex-col gap-2 pr-2 md:pr-3 lg:pr-4">
          <div className="flex items-center gap-2">
            ...
            {/* ── Desktop: Filter + Download inline ── [NEW] ── */}
            <div className="hidden md:flex items-center gap-2 shrink-0 h-full">
              {FilterDropdown}
              {DownloadButton}
            </div>

            {/* ── Mobile: 3-dot popover trigger ── [NEW] ── */}
            <div className="flex md:hidden shrink-0">
              <Button
                onClick={(e) => mobileMenuRef.current?.toggle(e)}
                aria-label="More options"
                aria-haspopup="true"
                className="flex items-center justify-center h-full aspect-square bg-base-200! border! border-neutral! rounded-lg! text-base-content! transition-colors hover:border-primary!"
              >
                {/* 3-dot icon via lucide MoreVertical */}
              ...
              <OverlayPanel
                ref={mobileMenuRef}
                className="web-apis-mobile-menu bg-base-300! border! border-neutral! rounded-xl! shadow-lg p-3"
                pt={{
                  content: { className: "p-0!" },
                }}
              >
                <div className="flex flex-col gap-2 min-w-[200px]">
                  <p className="text-xs font-medium uppercase tracking-widest text-neutral-content font-content px-1 select-none">
                    Options
                  </p>
                  {/* Filter dropdown — full width in popover */}
                  <div className="w-full [&_.p-dropdown]:w-full">
                    {FilterDropdown}
                  </div>
                  {/* Download — full width in popover */}
              ...
              <section key={category.id} aria-labelledby={`cat-${category.id}`}>
                ...
                <div className="web-apis-grid grid gap-2 md:gap-3 lg:gap-4 grid-cols-1 sm:grid-cols-2 mdl:grid-cols-3 lg:grid-cols-4">
                  {cards.map(({ cardKey, component }) => (
                    // [NEW] data-card-key attribute for DOM scraper
                    <div key={cardKey} data-card-key={cardKey}>
                      {component}
                    </div>
                  ))}
                </div>
        ...
        {/* [NEW] ScrollTop — binds to this scrollable container via target="parent" */}
        <ScrollTop
          target="parent"
          threshold={400}
          className="p-3! rounded-full bg-primary! text-white flex!"
          icon="pi pi-arrow-up"
        />
      </div>
    </Layout>
